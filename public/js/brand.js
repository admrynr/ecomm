...
var user = {
			...
			columns: [
				...
				}},
				{
					...
					render: function(data, type, row){
						...
						var button = "<button type='button' data-id='"+data.id+"' class='btn dotip btn-success btn-outline btn-circle m-r-5 btn-edt-data' data-toggle='tooltip' title='Edit Brand'>"
	...
	handleShowEditForm : function(data){
		...
		modal.modal("show");
		modal.find(".modal-title").text("Edit Data Brand");
		...
		$('#btn-bulk').on('click',function(){
			$.ajax({
				...
				success: function(data){
					...
					notification._toast('Success', 'Success Edit Brand', 'success');
					user.handleTable($('#filter').val());
				}
			});
		})
	...
	handleDeleteData : function(){
		$("#dataTableBrand tbody").on("click", ".btn-delete-data", function(){
		...
		$('#btn-hapus').on('click',function(){
			$.ajax({
				...
				success: function(data){
					...
					notification._toast('Success', 'Success Delete Brand', 'success');
					user.handleTable($('#filter').val());
				}
			});
		});
